...
class _LangConfig {
  ...
  const _LangConfig({
    ...
    required this.ttsLocale, required this.sttLocaleId,
  });
}

const List<_LangConfig> _kLanguages = [
  _LangConfig(code:'en', name:'English',  flag:'EN', ttsLocale:'en-IN', sttLocaleId:'en_IN'),
  _LangConfig(code:'hi', name:'Hindi',    flag:'HI', ttsLocale:'hi-IN', sttLocaleId:'hi_IN'),
  _LangConfig(code:'mr', name:'Marathi',  flag:'MR', ttsLocale:'mr-IN', sttLocaleId:'mr_IN'),
  _LangConfig(code:'gu', name:'Gujarati', flag:'GU', ttsLocale:'gu-IN', sttLocaleId:'gu_IN'),
  _LangConfig(code:'ta', name:'Tamil',    flag:'TA', ttsLocale:'ta-IN', sttLocaleId:'ta_IN'),
  _LangConfig(code:'te', name:'Telugu',   flag:'TE', ttsLocale:'te-IN', sttLocaleId:'te_IN'),
  _LangConfig(code:'kn', name:'Kannada',  flag:'KA', ttsLocale:'kn-IN', sttLocaleId:'kn_IN'),
  _LangConfig(code:'bn', name:'Bengali',  flag:'BN', ttsLocale:'bn-IN', sttLocaleId:'bn_IN'),
];
...
class _Message {
  ...
  const _Message({
    ...
    this.isSign = false, this.isVoice = false,
  });
}
...
class _TwoWayScreenState extends State<TwoWayScreen> with TickerProviderStateMixin {
  ...
  void _startFrameStream() {
    ...
    _frameTimer = Timer.periodic(
        Duration(milliseconds: _kFrameIntervalMs), (_) async {
  ...
  void _onSignReceived(dynamic data) {
    ...
    if (now.difference(_candidateSince).inMilliseconds < _kStableDetectionMs) return;
    if (now.difference(_lastAcceptedAt).inMilliseconds < _kDetectionCooldownMs) return;
    if (sign == _lastAccepted &&
        now.difference(_lastAcceptedAt).inMilliseconds < _kSameLabelCooldownMs) return;
    if (sign == _pending) return;
    _lastAccepted   = sign;
    _lastAcceptedAt = now;
    setState(() { _pending = sign; _detecting = false; });
  }
  ...
  void _addMessage(String text, _Sender sender,
      {bool isSign = false, bool isVoice = false}) {
    setState(() => _messages.add(_Message(
        text: text, sender: sender, time: DateTime.now(),
        isSign: isSign, isVoice: isVoice)));
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
              ...
              child: Text(l.t('bridge_clear'),
                  style: _body(14, isDark ? _dangerDark : _danger,
                      w: FontWeight.w700))),
        ],
      ),
    );
    ...
    return Stack(fit: StackFit.expand, children: [
      ...
      Positioned(top: 0, left: 0, right: 0,
          child: Container(
            ...
            child: Row(children: [
              ...
              const Spacer(),
              // Camera toggle
              Semantics(label: _camActive ? 'Pause camera' : 'Resume camera',
                  button: true,
                  child: _MobileTopBtn(
              ...
              if (_cameras.length > 1) ...[
                const SizedBox(width: _sp8),
                Semantics(label: l.t('bridge_flip_camera'), button: true,
                    child: _MobileTopBtn(
                        icon: Icons.flip_camera_ios_rounded, onTap: _flipCamera)),
              ],
            ]),
          )),
      ...
      if (_pending.isNotEmpty && _camActive)
        Positioned(top: 72, left: 0, right: 0,
            child: Center(child: _PendingSignChip(
                sign: _pending, isDark: true,
                onConfirm: _confirmSign, onDismiss: _clearPending,
                l: AppLocalizations.of(ctx)))),

      // Detecting badge
      if (_detecting && _camActive)
        Positioned(top: 72, left: _sp12,
            child: AnimatedBuilder(animation: _pulse, builder: (_, __) =>
                _DetectingBadge(pulse: _pulse))),
  ...
  Widget _mobileCamPlaceholder() => Container(
    ...
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Text(AppLocalizations.of(context).t('bridge_camera_init'),
  ...
  Widget _deafPanelHeader(bool isDark) {
    ...
    return Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(l.t('bridge_deaf_label'), style: _label(13, textClr, w: FontWeight.w700)),
            Text(l.t('bridge_deaf_sublabel'), style: _body(11, subClr)),
          ])),
      // WS status
      AnimatedBuilder(animation: _pulse, builder: (_, __) =>
          _ConnectionChip(
              connected: _wsConnected, pulse: _pulse,
              label: _wsConnected ? l.t('bridge_isl_live')
                  : (_reconnectAttempts < _maxReconnectAttempts
                  ? l.t('common_connecting') : 'Offline'),
              dark: isDark)),
      const SizedBox(width: _sp8),
      // Camera controls
      Semantics(label: _camActive ? 'Pause camera' : 'Resume camera', button: true,
      ...
      if (_cameras.length > 1) ...[
        ...
        Semantics(label: AppLocalizations.of(context).t('bridge_flip_camera'), button: true,
            child: _IconActionBtn(
            ...
            Container(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Container(
                    ...
                    child: Icon(Icons.sign_language_rounded, color: accent, size: 28)),
                const SizedBox(height: _sp12),
                Text(l.t('bridge_camera_init'), style: _body(12, subClr)),
                const SizedBox(height: _sp4),
                Text(l.t('bridge_camera_hint'), style: _body(11, isDark ? _dTextMuted : _lTextMuted)),
              ]),
            ),
                  ...
                  child: Column(mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.videocam_off_rounded,
                            color: isDark ? _dTextMuted : _lTextMuted, size: 32),
  ...
  Widget _signStatusBar(bool isDark) {
    ...
    return AnimatedContainer(
      ...
      child: Row(children: [
        Icon(hasPending ? Icons.record_voice_over_rounded : Icons.front_hand_rounded,
            color: hasPending ? accent : (isDark ? _dTextMuted : _lTextMuted),
            size: 18),
        const SizedBox(width: _sp12),
        Expanded(child: Text(
            hasPending ? _pending
                : (_wsConnected ? l.t('bridge_waiting_sign')
                : l.t('bridge_connect_backend')),
            style: _label(hasPending ? 14 : 12,
                hasPending ? accent : subClr,
                w: hasPending ? FontWeight.w700 : FontWeight.w400))),
        if (hasPending) ...[
          Semantics(label: l.t('bridge_confirm_sign'), button: true,
              child: InkWell(
                borderRadius: BorderRadius.circular(8),
                onTap: _confirmSign,
                child: Container(
          ...
          const SizedBox(width: _sp8),
          Semantics(label: l.t('bridge_dismiss_sign'), button: true,
              child: InkWell(
                borderRadius: BorderRadius.circular(6),
                onTap: _clearPending,
                child: Icon(Icons.close_rounded,
                    color: isDark ? _dTextMuted : _lTextMuted, size: 16),
              )),
        ],
      ]),
    );
  ...
  Widget _conversationHeader(bool isDark) {
    ...
    return Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(l.t('bridge_convo_title'), style: _label(13, textClr, w: FontWeight.w700)),
            Text(_messages.isEmpty
      ...
      if (_messages.isNotEmpty) ...[
        const SizedBox(width: _sp8),
        Semantics(label: l.t('bridge_clear_convo'), button: true,
            child: _IconActionBtn(
                icon: Icons.delete_sweep_rounded,
                color: isDark ? _dTextSub : _lTextSub,
                isDark: isDark, onTap: _clearChat)),
      ],
    ]);
  }
    ...
    return Container(
      ...
      child: _messages.isEmpty
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
                ...
                decoration: BoxDecoration(
                    color: isDark ? _dSurface2 : _lSurface2,
                    shape: BoxShape.circle),
                child: Icon(Icons.forum_outlined, color: mutedClr, size: 26)),
            const SizedBox(height: _sp12),
          ...
          child: ListView.builder(
            controller: _scroll,
            padding: const EdgeInsets.symmetric(
                horizontal: _sp12, vertical: _sp16),
            itemCount: _messages.length,
            itemBuilder: (_, i) => _MessageBubble(
                msg: _messages[i], isDark: isDark,
                onSpeak: _speak, ttsSpeaking: _ttsSpeaking),
          )),
    );
  }
  ...
  Widget _hearingInputBar(bool isDark) {
    ...
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Row(children: [
        ...
        Semantics(label: l.t('bridge_quick_phrases'), button: true,
            child: InkWell(
              borderRadius: BorderRadius.circular(8),
      ...
      Row(children: [
          ...
          child: AnimatedContainer(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText:  _listening
                    ? '${l.t('isl_input_listening')} (${_lang.name})'
        ...
        const SizedBox(width: _sp8),
        // Send button
        Semantics(label: l.t('common_send_message'), button: true,
            child: InkWell(
              borderRadius: BorderRadius.circular(22),
              onTap: _sendHearing,
              child: AnimatedContainer(
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text(l.t(p.key),
                      style: _body(11, subClr), maxLines: 2,
...
class _MessageBubble extends StatelessWidget {
  ...
  const _MessageBubble({required this.msg, required this.isDark,
    required this.onSpeak, required this.ttsSpeaking});

  @override
  Widget build(BuildContext context) {
    final l        = AppLocalizations.of(context);
    final isDeaf   = msg.sender == _Sender.deaf;
    final accent   = isDeaf ? Color(0xFFA78BFA) : (isDark ? _secondaryDark : _secondary);
    ...
    return Semantics(
        label: '${isDeaf
          ? l.t('bridge_message_source_sign')
          : l.t('bridge_message_source_voice')}: ${msg.text}',
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment:
          isDeaf ? CrossAxisAlignment.start : CrossAxisAlignment.end,
          children: [
            // Role row
            Padding(
              padding: const EdgeInsets.only(bottom: _sp4, left: _sp4, right: _sp4),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                mainAxisAlignment:
                isDeaf ? MainAxisAlignment.start : MainAxisAlignment.end,
                children: [
                  Icon(
                      isDeaf ? Icons.sign_language_rounded
                          : (msg.isVoice ? Icons.mic_rounded : Icons.keyboard_rounded),
                      size: 10, color: accent.withOpacity(0.7)),
                  const SizedBox(width: _sp4),
                  Text(isDeaf ? l.t('bridge_deaf_label') : l.t('bridge_hearing_label'),
                      style: _label(10, accent.withOpacity(0.7), w: FontWeight.w600)),
                ],
              ),
            ),
            // Bubble
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    Row(children: [
                      Text(timeStr, style: _label(9, timeClr, w: FontWeight.w400)),
                      const Spacer(),
                      // Speak button
                      Semantics(
                        label: l.t('common_speak'), button: true,
                        child: InkWell(
                          borderRadius: BorderRadius.circular(6),
...
class _MobileBridgePanel extends StatefulWidget {
  ...
  const _MobileBridgePanel({
    required this.isDark, required this.listening, required this.ttsSpeaking,
    required this.autoSpeak, required this.speechOk, required this.messages,
    required this.typeCtrl, required this.typeFocus, required this.typeFocused,
    required this.scrollCtrl, required this.pulse, required this.selectedLangCode,
    required this.onSendHearing, required this.onVoiceInput, required this.onStopSpeak,
    required this.onToggleAutoSpeak, required this.onClearChat,
    required this.onPhraseSelected, required this.onLangChanged,
  });
...
class _MobileChatTab extends StatelessWidget {
  ...
  const _MobileChatTab({
    required this.messages, required this.typeCtrl, required this.typeFocus,
    required this.typeFocused, required this.isDark, required this.scrollCtrl,
    required this.onSend, required this.onVoice, required this.speechOk,
    required this.listening, required this.micColor,
  });
    ...
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      SizedBox(
        ...
        child: messages.isEmpty
          ...
          const SizedBox(height: _sp8),
          Text(l.t('bridge_empty_sub'),
              style: _body(11, subClr), textAlign: TextAlign.center),
        ]))
            : ListView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              final isDeaf = msg.sender == _Sender.deaf;
              final ac     = isDeaf ? Color(0xFFA78BFA)
              ...
              return Padding(
                padding: EdgeInsets.only(
                    bottom: _sp8, left: isDeaf ? 0 : 24, right: isDeaf ? 24 : 0),
                child: Container(
                  padding: const EdgeInsets.symmetric(
                      horizontal: _sp12, vertical: _sp8),
                  decoration: BoxDecoration(
                      ...
                      border: Border.all(color: ac.withOpacity(0.25), width: 1)),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        if (msg.isSign)
                          Container(
                        ...
                        Text(msg.text, style: _body(13, textClr, w: FontWeight.w500)),
                        const SizedBox(height: _sp4),
                        Text(ts, style: _label(9, ac.withOpacity(0.5), w: FontWeight.w400)),
                      ]),
                ),
              );
            }),
      ...
      Padding(
        ...
        child: Row(children: [
          // Mic
          if (speechOk)
            Semantics(label: listening ? 'Stop voice' : 'Voice input', button: true,
                child: InkWell(
                  borderRadius: BorderRadius.circular(22),
                  onTap: onVoice,
                  child: Container(
                      width: 44, height: 44,
                      decoration: BoxDecoration(
                          color: micBg, shape: BoxShape.circle,
                          border: Border.all(
                              color: listening
                                  ? micColor.withOpacity(0.35) : border,
                              width: listening ? 1.5 : 1.0)),
                      child: Icon(listening ? Icons.mic_rounded : Icons.mic_none_rounded,
                          size: 18, color: micColor)),
                )),
          ...
          Expanded(child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
                ...
                border: Border.all(
                    color: typeFocused
                        ? accent.withOpacity(0.45) : border,
                    width: typeFocused ? 1.5 : 1.0)),
            child: TextField(
              controller: typeCtrl, focusNode: typeFocus,
              style: _body(14, textClr),
                ...
                child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                        color: typeCtrl.text.isEmpty
                            ? (isDark ? _dSurface2 : _lSurface2) : accent,
                        shape: BoxShape.circle,
                        border: Border.all(
...
class _MobilePhrasesTab extends StatelessWidget {
  ...
  const _MobilePhrasesTab(
      {required this.isDark, required this.onPhraseSelected});
...
class _MobilePanelTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(selected: active, button: true, label: label,
...
class _ConnectionChip extends StatelessWidget {
  ...
  final Animation<double> pulse;
  final String label;
  const _ConnectionChip({required this.connected, required this.pulse,
    required this.label, required this.dark});
...
class _PendingSignChip extends StatelessWidget {
  ...
  const _PendingSignChip({required this.sign, required this.isDark,
    required this.onConfirm, required this.onDismiss, required this.l});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.sign_language_rounded, color: const Color(0xFFA78BFA), size: 18),
        const SizedBox(width: _sp8),
        Flexible(child: Text(sign,
            style: _heading(20, Colors.white))),
        const SizedBox(width: _sp12),
        Semantics(label: l.t('bridge_confirm_sign'), button: true,
            child: GestureDetector(
              onTap: onConfirm,
    ...
    return AnimatedBuilder(
        ...
        builder: (_, __) => Container(
          ...
          decoration: BoxDecoration(
              ...
              border: Border.all(
                  color: accent.withOpacity(0.3 + pulse.value * 0.5), width: 1)),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Container(width: 6, height: 6,
                decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(
                        ...
                        blurRadius: 7)])),
            const SizedBox(width: _sp4),
            Text(AppLocalizations.of(context).t('bridge_detecting'),
